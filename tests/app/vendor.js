	...
	Module.prototype = {
		...
		*/
		determine: function(url) {
			if (/^[$a-z0-9\-_]+\/\//i.test(url) && "string"===typeof Vendor.config.aliases[url.split('//')[0]]) {
				/* Alias */
				var alias = url.split('//')[0];
				/*
				Теперь формируем путь
				*/
				url = Vendor.config.aliases[alias]+(Vendor.config.aliases[alias].substr(-1)==='/' ? '' : '/')+url.substr(alias.length+2);
			}
	...
	*/
	var Vendor = function(resources, callback) {
		...
		*/
		...
		new Module('Unnamed', resources, callback, caller, caller);
	}
			...
			for (var j in g) {
				...
				if (srci!==false) {
					...
					var importedConfig={
						...
						requireAlias: g[j].getAttribute('require-alias') ? g[j].getAttribute('require-alias') : 'require',
					};
					/*
					Анализируем контент внутри тэга
					*/
					try {
						eval('importedConfig = mixin(importedConfig, '+g[j].innerText+')');
					} catch(e) {
						// do nothing
					}
