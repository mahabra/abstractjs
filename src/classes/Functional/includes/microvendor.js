define(function() {
	...
	*/
	return (function() {
		...
		var Vendor = function(userresources, callback) {
				...
				this.determine = function(path) {
					...
					if (Vendor.constants.imagesRegExpr.test(path)) {
						...
						Vendor.images(path, function(module) {
							that.queue[q] = module;
							that.loaded.apply(that, arguments);
						});
						return;
					}
		...
		Vendor.optional = function(config) {
		...
		// Events support
		Vendor.eventListners = {};
		Vendor.bind = function(e, callback, once) {
				var once = once;
		...
		Vendor.require = function(c, b, phantom) {
					...
					for (var i = 0; i < this.src.length; i++) {
						...
						*/
						var upload_module = function(f) {
							...
							/* Нормализуем url */
							var normalsrc = normalize((!/^http/.test(k) ? Vendor._config.baseUrl : "") + k + (function(l) {
								if (l != "") {
									return "?" + l
								}
								return ""
							})(Vendor._config.urlArgs));
							...
							try {
								...
								if (Vendor._defined.indexOf(normalsrc) > -1) {
							...
							if ('undefined' !== typeof Vendor.define._modules[normalsrc]) {
							...
							if (Vendor._detained.indexOf(normalsrc)>-1) {
								...
								Vendor._onRedetain(normalsrc, function() {
									...
									if ("undefined" == typeof Vendor.define._modules[normalsrc])
									if (Vendor._defined.indexOf(normalsrc)<0) Vendor._defined.push(normalsrc);
						              	...
						              	if (!this.readyState || this.readyState === "loaded" || this.readyState === "complete" || interactive) {
							              	...
							              	var ok = function(defi) {
								                ...
								                if ("undefined" == typeof Vendor.define._modules[normalsrc]) {
									                ...
									                Vendor.define._modules[normalsrc] = {
									                  fabric: defi || null
									                };  
								            	};
								                ...
								                if (Vendor._defined.indexOf(normalsrc)<0) 
								                {				                  
								                  	Vendor._defined.push(normalsrc);
								                };
							              	...
							              	try {
								            	...
								            	if (Vendor.debugMode) console.log('Vendor.js: script node is already removed by another script', j);
								            }
						               	 };
						              };
						...
						var f = (typeof Vendor._config.paths[this.src[i]] == "string") ? Vendor._config.paths[this.src[i]] : this.src[i];
					...
					if (this.loads < 1) {
						...
						if (typeof this.callback == "function") {
							...
							for (var g = 0; g < this.fabrics.length; g++) {
								...
								if ("undefined" != typeof Vendor.define._modules[this.fabrics[g]]) {
									
									h.push(null != Vendor.define._modules[this.fabrics[g]] ? Vendor.define._modules[this.fabrics[g]].fabric : null)
								} else {
		...
		Vendor.ext = Vendor.prototype = {
			selector: "",
			constructor: Vendor,
			init: function() {
				return Vendor.ext.constructor
			}
		};
		Vendor.selfLocationdefined = false; // Means we know our location
		Vendor.defineDataImport = false;
		Vendor._detained = [];
		Vendor._freezed = [];
		Vendor.queue = {

		};
		Vendor._onRedetain = function(d, f) {
			typeof Vendor.queue[d] != "object" && (Vendor.queue[d] = []);
			Vendor.queue[d].push(f);
		};
		Vendor.releaseDetained = function(d){
			
			var nd = [];
			for (var dt = 0;dt<Vendor._detained.length;dt++) {
				if (Vendor._detained[dt]!=d) nd.push(Vendor._detained[dt]);
			}
			Vendor._detained = nd;
			
			
			typeof Vendor.queue[d] == "object" && (function(d, q) {
		...
		Vendor._config = {
			baseUrl: "",
			urlArgs: "",
			bowerComponentsUrl: window.location.origin+"/bower_components/",
		...
		Vendor.brahmaInside = true;
		Vendor.config = function(f) {
			
			var f = f || {};
			if (typeof f.paths == "object") {
				for (var b in f.paths) {
					Vendor._config.paths[b] = f.paths[b]
				}
				delete f.paths
			}
			for (var a in f) {
				Vendor._config[a] = f[a]
			}
		}
			
		Vendor.define = function(g, e, b) {
			...
			var synchCode = Vendor.define.synchCode = Math.random();
		...
		Vendor.requirecss = function(b, f) {
		...
		Vendor.getJson = function(path, callback) {
		...
		Vendor.load = function(path, callback) {
		...
		! Путь, начинающийся с / всегда подставляется к домену сайта (window.location.origin)
		...
		*/
		Vendor.makeAbsPath = function(path, baseUrl) {
			var absloc = (path.length>0) ? (path + (path.substr(-1, 1)==='/' ? '' : '/')) : path;
			(absloc.substr(0,4).toLowerCase()!='http') && ( (absloc.substring(0,1)==='/') ? (absloc = window.location.origin+absloc) : (absloc = (baseUrl||Vendor._config.baseUrl)+absloc));
			return absloc;
		}
		...
		*/
		Vendor.makeAbsFilePath = function(fn, baseUrl) {
		...
		window.bower = Vendor.bower = function(r, callback) {
			...
			if (!Vendor._config.noBowerrc) {
				Vendor.getJson(window.location.origin+'/.bowerrc', function(data) {
					if ("object"===typeof data && "string"===typeof data.directory) {
						
						Vendor._config.bowerComponentsUrl = Vendor.makeAbsPath(data.directory, window.location.origin+'/');
					}
					run.call(this);
				})
			} else {
		...
		Vendor.images = function(imgs, callback) {
			var progressor = new (function(imgs, callback) {
				...
				this.execute = function() {
					...
					for (var i=0;i<this.imgs.length;i++) {
						...
						this.loadImage(absloc, this.imgs[i]);
					}

					this.max = this.loadings;
				}
				this.loadImage = function(src, userSrc) {
					this.loadings++;
					var that = this,				
					img = new Image();
					...
					img.onload = img.onerror = function() {
						...
						that.loaded(img, userSrc);
					}
					this.images.push(img);
					img.src = src;
				};
				this.loaded = function(res, userSrc) {
					this.loadings--;
					this.progress = 1-(this.loadings/this.max);

					if (this.eachCallback) this.eachCallback.call(this, res, userSrc);
		...
		if (typeof Vendor === "function" && typeof Vendor.brahmaInside === "boolean") {
